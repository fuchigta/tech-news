...
export default function TagsBoard() {
  ...
  const [excludeTags, setExcludeTags] =
    useState<string>("あとで読む *あとで読む 未分類");
  ...
  return (
    <Card className="flex flex-col justify-center h-full w-full">
      <CardHeader className="text-left">
        ...
        <div>
          <InputLabel htmlFor="minTagged">
            タグ付けされたエントリを含むフィード数がN件以上
          </InputLabel>
          <Input
            id="minTagged"
            type="number"
            className="w-16"
            value={minTagged.toString()}
            onChange={(e) => {
                          ...
                          {tag.feeds.toArray().map((f) => (
                            <Card key={f.feed_url}>
                              ...
                              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
                                {tag.entries
                                  .toArray()
                                  .filter((e) => e.feed_title == f.feed_title)
                                  .map((e) => (
                                    <Card
                                      key={e.entry_url}
                                      className="flex flex-col md:flex-row hover:bg-muted/50 cursor-pointer"
                                      onClick={() => {
                                        const w = window.open(
                                          e.entry_url,
                                          "_blank",
                ...
                <CardContent className="grow flex items-center">
                  ...
                  >
                    <PieChart>
                      ...
                      />
                      <Pie
                        data={tag.feeds.toArray().map((f, i) => ({
                          ...f,
